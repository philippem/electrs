...
impl DB {
    ...
    pub fn enable_auto_compaction(&self) {
        // Reset L0 triggers to RocksDB defaults so steady-state operation
        // compacts promptly rather than letting 64 L0 files accumulate.
        let opts = [
    ...
    pub fn iter_scan_reverse(&self, prefix: &[u8], prefix_max: &[u8]) -> ReverseScanIterator<'_> {
        // prefix_max is always >= prefix length. When >= 33 bytes, the default
        // raw_iterator operates in prefix mode: seek_for_prev uses the prefix bloom
        // to skip SST files, and .prev() stays within the extracted prefix boundary.
        // ReverseScanIterator also enforces the prefix via starts_with().
        // When < 33 bytes, use total_order_seek to avoid silent misses.
        let mut iter = if prefix_max.len() >= 33 {
